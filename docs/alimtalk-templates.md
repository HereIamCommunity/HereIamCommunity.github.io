...
공통 규칙
- 변수는 `#{이름}` 형식. **코드가 넣는 변수 키와 글자 단위로 정확히 일치해야 한다.** 등록 시 변수명을 문서와 다르게 썼다면 `src/lib/kakao.ts`의 해당 함수 variables 키를 등록한 이름으로 맞춘다.
...
승인되면 Vercel `KAKAO_TEMPLATE_SALON_CONFIRMED=eQ47xS1xvH` 입력.
