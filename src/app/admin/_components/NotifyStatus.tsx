...
export default function NotifyStatus({
  ...
  return (
    <span className={`inline-flex items-center gap-2 whitespace-nowrap ${compact ? "w-full" : ""}`}>
      <span className={`inline-flex items-center gap-1.5 whitespace-nowrap ${compact ? "min-w-0" : ""}`}>
        <span aria-hidden="true" className={`w-1.5 h-1.5 rounded-full shrink-0 ${DOT[kind]}`} />
        <span
          title={compact ? text : undefined}
          className={`text-xs whitespace-nowrap ${TEXT[kind]} ${compact ? "overflow-hidden text-ellipsis" : ""}`}
        >
          {text}
        </span>
      </span>
      {canResend && (
        <button
          ...
          disabled={busy}
          className={`${BTN_SMALL} shrink-0 border-orange-dark bg-orange/10 text-brown hover:bg-orange/20`}
        >
          {busy && <Spinner />}
          재발송
        </button>
