...
import { applyPatches } from "@/lib/store";
...
export async function POST(req: NextRequest) {
  ...
  // 읽기 실패와 "기록 없음"을 구분한다. DB 오류를 404로 보여주면 운영자가
  ...
  try {
    ...
    return NextResponse.json(
      { ok: false, error: "실행 기록을 읽지 못했습니다(DB 오류). 잠시 후 다시 시도해주세요." },
      { status: 500 }
    );
  }
  ...
  try {
    ...
    const updated = await applyPatches(planRevertWrites(entry.snapshot));
    const at = kstTimestamp();
    try {
      await markBulkLogReverted(entry.id, at);
    } catch (e) {
